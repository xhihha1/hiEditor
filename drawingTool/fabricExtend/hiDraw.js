...
hiDraw.prototype.uniqueIdGenerater = function (action) {
    ...
    if (action == 'delete') {
        ...
        this.canvasView.forEachObject(function (obj) {
            if(obj.get('uniqueId')){
                that.labelObjsIndexList.push(obj.get('uniqueIndex'))
            }
        })
    } else {
        var id = 'I_' + new Date().getTime()
        while(that.labelObjsIndexList.indexOf(id) > 0){

        }
        that.labelObjsIndexList.push(id)
    }
...
hiDraw.prototype.colorToRgbA = function (color, alpha) {
    ...
    if(/^#([A-Fa-f0-9]{3}){1,2}$/.test(color)){
        ...
        c= '0x'+c.join('');
        if(typeof(alpha) !== 'number' && !alpha){alpha = 1;}
        ...
        if(typeof(alpha) !== 'number' && !alpha){m[3] = 1;}
        else {m[3] = alpha;}
